...
The recommended way to install FindCursor is via [Package Control][package-control].  Package Control will install the correct branch on your system and keep it up to date.
...
!!! warning "Warning"
    This is not the recommended way to install FindCursor for the average user.  Installing this way **will not** get automatically updated.

    If you are forking for a pull request, you should **just** clone FindCursor and run Package Control's `Satisfy Dependency` command to get all the dependencies.
...
2. Download and unpack, or git clone, the latest FindCursor release and unpack as `FindCursor`:
    ...
    - https://github.com/facelessuser/FindCursor -> FindCursor
